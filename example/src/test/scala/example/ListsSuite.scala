...
class ListsSuite extends FunSuite {
  ...
  test("sum of single item list is that item") {
    assert(sum(List(5)) === 5)
  }

  test("sum of a couple of numbers") {
  ...
  test("max of single item list is that item") {
    assert(max(List(5)) === 5)
  }

  test("max of a couple of numbers") {
    assert(max(List(2, 4)) === 4)
  }
